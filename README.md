...
|-----------|-----------|--------|
| Network | Terraform (Project 1) | Separate lifecycle from workloads |
| Kubernetes | EKS Managed | AWS-managed control plane, reduced ops overhead |
| Ingress | AWS ALB (Load Balancer Controller) | Native AWS integration, WAF support, TLS termination |
| Secrets | AWS Secrets Manager + External Secrets Operator | No secrets in Git or etcd; auto-rotation support |
| Keycloak | Custom Helm Chart | Full control, blue-green support, custom probes |
| Database | PostgreSQL StatefulSet | Self-contained for this assignment; RDS recommended for production |
| Observability | CloudWatch Container Insights + Fluent Bit | AWS-native, zero additional cost for basic metrics |
| Deployment | Blue-Green via slot labels | Zero-downtime, instant rollback |
...
│
├── project1-network/             # Terraform Project 1 — VPC + networking
│   ├── main.tf                   # Root: composes vpc, subnets, security-groups
│   ├── variables.tf
│   ├── outputs.tf
│   ├── environments/
│   │   └── prod/
...
│       ├── subnets/              # Public/private subnets, NAT GWs, route tables
│       └── security-groups/      # EKS cluster and node security groups
│
├── project2-k8s/                 # Terraform Project 2 — EKS + services
│   ├── main.tf                   # Root: reads P1 state, provisions cluster + Helm
│   ├── variables.tf
│   ├── outputs.tf
│   ├── environments/
│   │   └── prod/
│   │       └── terraform.tfvars
│   └── modules/
│       ├── eks/                  # EKS cluster, node group, OIDC, add-ons, KMS
│       ├── irsa/                 # IAM roles for ALB controller, ESO, CloudWatch
│       ├── secrets/              # AWS Secrets Manager secrets (auto-generated)
...
│       └── templates/
│           ├── _helpers.tpl
│           ├── deployment.yaml   # Blue-green aware Deployment
│           ├── service.yaml      # ClusterIP + headless
│           ├── ingress.yaml      # AWS ALB ingress
│           ├── postgresql.yaml   # StatefulSet + services
│           ├── external-secrets.yaml  # SecretStore + ExternalSecret CRDs
│           └── serviceaccount-hpa-pdb.yaml
│
└── manifests/
    ├── blue-green/
    │   └── service-switch.yaml   # Reference manifest for slot switching
    └── external-secrets/
